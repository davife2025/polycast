...
async function marketsRoutes(app) {
    ...
    app.get("/markets/:id/events", async (request, reply) => {
        const { id } = request.params;
        const { data, error } = await supabase_1.supabaseAdmin
            .from("market_events")
            .select("*")
    ...
    app.get("/markets/by-address/:address/events", async (request, reply) => {
        const address = request.params.address.toLowerCase();
        const { data: market, error: marketError } = await supabase_1.supabaseAdmin
            .from("markets")
            .select("id")
